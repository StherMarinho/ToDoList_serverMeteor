...
class SegurancaApi {
	_getRecursosUsuario(user: IUserProfile | (Meteor.User & { roles?: string[] | undefined }) | undefined): Set<Recurso> {
		if ((user as IUserProfile | undefined)?.status === 'disabled') return new Set<string>();
		let roles = user && user.roles;
		if (!roles) roles = ['Public'];
		const resources = new Set<string>();

		roles.forEach((role) => {
			if (!Object.values(RoleType).includes(role as RoleType)) return;
			const recursosRole = mapRolesRecursos[role as RoleType];
			if (recursosRole) {
				recursosRole.forEach((resource: string) => resources.add(resource));
			}
		});
		return resources;
	}
